...
@interface YXRequestTask () <NSURLConnectionDataDelegate, NSURLSessionDataDelegate>
...
@implementation YXRequestTask

- (instancetype)init {
    
    if (self = [super init]) {
        [YXFileHandle createTempFile];
    }
    return self;
}
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler {
    ...
    self.fileLength = (NSUInteger)(fileLength.integerValue > 0 ? fileLength.integerValue : response.expectedContentLength);
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data {
    ...
    if (self.cancel) return;
    [YXFileHandle writeTempFileData:data];
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    ...
    if (self.cancel) {
        ...
        if (error) {
            ...
            //可以缓存则保存文件
            if (self.cache) {
                [YXFileHandle cacheTempFileWithFileName:[NSString fileNameWithURL:self.requestURL]];
            }
